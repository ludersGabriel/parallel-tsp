...
int run_tsp() {
  ...
  struct timeval tsp_start, tsp_end;
  ...
  // start of my parallel region in the parallel code
  gettimeofday(&tsp_start, NULL);
  tsp(1, 0, path);
  gettimeofday(&tsp_end, NULL);

  tsp_time = (tsp_end.tv_sec - tsp_start.tv_sec) +
             (tsp_end.tv_usec - tsp_start.tv_usec) / 1000000.0;
  // end of my parallel region in the parallel code
...
int main(int argc, char **argv) {
  ...
  double elapsed_time, pure_seq_time;
  ...
  pure_seq_time = elapsed_time - tsp_time;
